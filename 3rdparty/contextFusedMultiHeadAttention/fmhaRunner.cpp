...
FusedMHARunnerV2::FusedMHARunnerV2(MHARunnerFixedParams fixedParams): mFixedParams(fixedParams) {
    RTP_LLM_CHECK_WITH_INFO((mSM == kSM_80 || mSM == kSM_86 || mSM == kSM_89 || mSM == kSM_90 || mSM == kSM_100
                             || mSM == kSM_120 || mSM == kSM_121),
                            "Unsupported architecture");
    RTP_LLM_CHECK_WITH_INFO((mFixedParams.dataType == DATA_TYPE_FP16 || mFixedParams.dataType == DATA_TYPE_BF16
...
void FusedMHARunnerV2::setupKernelParams(MHARunnerParams runnerParams) {
    ...
    if (mLaunchParams.attention_mask_type == ContextAttentionMaskType::SLIDING_OR_CHUNKED_CAUSAL
        ...
        RTP_LLM_CHECK_WITH_INFO((runnerParams.chunkedAttentionSize & (runnerParams.chunkedAttentionSize - 1)) == 0,
    ...
    RTP_LLM_CHECK_WITH_INFO(mFixedParams.numQHeads % mFixedParams.numKvHeads == 0,
    ...
    if (mFixedParams.attentionMaskType == ContextAttentionMaskType::CUSTOM_MASK) {
        ...
        mKernelParams.packed_mask_stride_in_bytes =
            (rtp_llm::div_up(int64_t(runnerParams.kvSeqLen), int64_t(FLASH_ATTEN_PACKED_MASK_N_ALIGNMENT))
    ...
    RTP_LLM_CHECK_WITH_INFO(runnerParams.attentionSinksPtr == nullptr || mSM == kSM_90,
    ...
    if (mFixedParams.dataType == DATA_TYPE_E4M3) {
        ...
        if (runnerParams.scaleBmm1Ptr == nullptr) {
            mKernelParams.scale_bmm1_d = nullptr;
        }
    }
...
void FusedMHARunnerV2::setupLaunchParams(MHARunnerParams runnerParams) {
    ...
    RTP_LLM_CHECK_WITH_INFO(mFixedParams.headSize > 0, "Head size should be greater than 0.");
    ...
    if ((runnerParams.kvSeqLen > runnerParams.slidingWindowSize
        ...
        RTP_LLM_CHECK_WITH_INFO(!(runnerParams.kvSeqLen > runnerParams.chunkedAttentionSize
        ...
        RTP_LLM_CHECK_WITH_INFO(isSm90 || runnerParams.kvSeqLen <= runnerParams.chunkedAttentionSize,
    ...
    if (isSm90 && (mFixedParams.dataType == DATA_TYPE_E4M3 || (separateQKvInput && runnerParams.kvSeqLen > 512))) {
        ...
        RTP_LLM_CHECK_WITH_INFO(false, "Unsupported architecture");
    }
    ...
    } else {  // always use flash attention kernels for Ampere/Ada
        mLaunchParams.flash_attention = true;
    ...
    if (!isSm90) {
        ...
        mLaunchParams.supportReturnSoftmaxStats =
            (runnerParams.softmaxStatsPtr != nullptr && mLaunchParams.flash_attention
...
void FusedMHARunnerV2::setTmaDescriptors(MHARunnerParams runnerParams) {
    ...
    cudaTmaDescSwizzle const swizzle_mode =
        (d_bytes_per_group > 64 ?
    ...
    if ((get_size_in_bytes(mFixedParams.dataTypeOut) == 1)
        ...
        qo_tma_descriptor.set_tma_desctriptor(o_ptr,
                                              desc_format,
                                              cudaTmaDescInterleave::INTERLEAVE_DISABLED,
                                              swizzle_mode,
                                              cudaTmaDescPromotion::PROMOTION_DISABLED,
    ...
    if (layout == AttentionInputLayout::Q_PAGED_KV) {
        ...
        RTP_LLM_CHECK(kv_step % tokens_per_block == 0 || tokens_per_block % kv_step == 0);
        ...
        // K
        kv_tma_descriptor.set_tma_desctriptor(kv_ptr,
                                              desc_format,
                                              cudaTmaDescInterleave::INTERLEAVE_DISABLED,
                                              swizzle_mode,
                                              cudaTmaDescPromotion::PROMOTION_DISABLED,
        ...
        // K
        kv_tma_descriptor.set_tma_desctriptor(k_ptr,
                                              desc_format,
                                              cudaTmaDescInterleave::INTERLEAVE_DISABLED,
                                              swizzle_mode,
                                              cudaTmaDescPromotion::PROMOTION_DISABLED,
                                              ...
                                              &mKernelParams.tma_desc_k);
        // V
        kv_tma_descriptor.set_tma_desctriptor(v_ptr,
                                              desc_format,
                                              cudaTmaDescInterleave::INTERLEAVE_DISABLED,
                                              swizzle_mode,
                                              cudaTmaDescPromotion::PROMOTION_DISABLED,
